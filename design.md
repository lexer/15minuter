...
src/
  ...
  storage/
    ...
    AnalysisLogger.ts      — Per-tick JSON-lines analysis log (logs/analysis/YYYY-MM-DD_HH-MM/<ticker>.log)
  agent/
...
| `logs/analysis/YYYY-MM-DD_HH-MM/<ticker>.log` | Per-tick JSON-lines analysis log (one file per ticker per 15-min window) |
| `btc_trade_history.json` | All trade records |

### 11. Logging
Analysis logs are written to `logs/analysis/YYYY-MM-DD_HH-MM/<ticker>.log` — one file per Kalshi market per 15-minute window (HH-MM = window close time in PST). One JSON-lines entry per **WebSocket ticker tick**, written for all subscribed markets at all times (not limited to the trading window). Each entry includes:
